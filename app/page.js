...
export default function PrepMasterApp() {
  ...
  async function loadBatches(page = 1, replace = false) {
    ...
    try {
      const r = await fetch(`/api/batches?page=${page}`, {
        cache: "no-store",
      });
    ...
    try {
      ...
      setAiMessages([
        ...next,
        {
          role: "assistant",
          content:
            "AI backend abhi configured nahi hai.",
        },
      ]);
    } finally {
      setLoadingAI(false);
            ...
            {batchPage < totalBatchPages && (
              ...
              >
                <button
                  ...
                  onClick={() =>
                    loadBatches(batchPage + 1)
                  }
                >
                  {loadingBatches
                    ? "Loading..."
                    : `Load More (${batchPage}/${totalBatchPages})`}
                </button>
              </div>
            )}
          </section>
      ...
      {tab === "my" && (
        <main className="page">
          ...
          {myBatches.length ? (
            ...
            <Empty
              text="Abhi koi batch enrolled nahi hai. Batches se ek batch enroll karo."
            />
          )}
        </main>
      )}
          ...
          <div className="chat">
            <div className="messages">
              {community.length ? (
                ...
                <Empty
                  text="Community me pehla message tum bhejo."
                />
              )}
            </div>
